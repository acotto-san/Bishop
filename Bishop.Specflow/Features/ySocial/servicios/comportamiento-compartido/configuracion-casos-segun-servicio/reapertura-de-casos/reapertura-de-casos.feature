...
Característica: Configuracion de reapertura de casos a nivel global
    Como supervisor 
    Quiero tener una configuracion en el servicio para la reapertura de casos
    Asi todos los servicios permiten la apertura de casos

    Antecedentes:
        Dado que en la configuracion de un servicio se encuentran las siguientes configuraciones:
            | atributo                     | valor      |
    ...
    Regla: los agentes van a poder reabrir casos si estan dadas las condiciones
        ...
        Escenario: agente puede reabrir un caso cerrado por un agente/supervisor
    ...
    Regla: hay escenarios donde no se esta en condiciones de reabrir el caso anterior
        ...
        Escenario: no se muestra modal de reapertura si el caso previo ya fue encuestado
            ...
            Cuando el caso "100" es asignado al agente poochie
            Entonces no vera la opcion de reabrir el caso "50"
        Escenario: no se muestra modal de reapertura si el caso previo ingreso por yflow
            Dado que el mensaje del caso "100" es enviado a yflow
            Y luego es derivado  a una ysocial
            Cuando el caso "100" es asignado al agente poochie
            Entonces no vera la opcion de reabrir el caso "50"
        Escenario: no se muestra modal de reapertura si el caso nuevo tiene un mensaje descartado
        ...
        Escenario: no se muestra modal de reapertura si el caso nuevo tiene un mensaje respondido
