#language: es
Característica: Tener un contador de tareas HSM enviadas
	Como supervisor que va a estar controlando los envíos de tareas
	Quiero tener un contador de tareas enviadas en determinado tiempo
	Para poder tener nocion de cuantas tareas van enviadas

	Regla de negocio: Son contabilizadas sin importar el usuario que agenda la tarea
		
		@tc:
		Esquema del escenario: Supervisor agenda tareas y se visualizan en el contador de tareas enviadas
		...
		@tc:
		Esquema del escenario: Integracion agenda por API tareas y se visualizan en el contador de tareas enviadas
			Dado que se esta viendo el contador desde
				| seccion | <seccion>    |
				| vista   | <tipo-tarea> |
			Y se ejecuto una <tipo-tarea> en la franja de tiempo comprendida por el contador 
	...
	Regla de negocio: Mientras hayan finalizado con fecha de inicio cuentan sin importar la razon de finalización
		...
		@tc:
		Esquema del escenario: Una tarea finaliza con fallo y se visualiza en el contador de tareas enviadas
